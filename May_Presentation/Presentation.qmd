...
A logit-transformation was applied (*extreme proportions*), variance was pooled using a generalised linear mixed model (*robust with sample sample sizes*), with study-level variances drawn from each study's observed proportion (*not assuming homogeneity across studies*).
...
- Explore CIs for Proportions in PG scenarios
- Discuss Simulation Study assessing CIs
- Examine Sample Size Calculation
- Present our Shiny App: PG-Power
- Discuss Final Steps
...
- CI method must be pre-specified in SAP submitted to authorities

- CANNOT be changed after seeing the data

- Drives sample size calculation

- For companies: n = \$

- For patients if device is effective: higher n = greater burden (time for approval)
...
**Complication Proportions** = near 0
:::
...
![](images/PG-Power-QR.png){width="418"}
