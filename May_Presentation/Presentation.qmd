---
title: "A Statistical Perspective of Non-Inferiority" 
...
# Section 1 {background-color="#5A3A6E"}

-   Discuss Non-Inferiority & Performance Goal Studies
-   Describe our Case Study
-   Determine Performance Endpoints

## Non-Inferiority RCT vs Performance Goal Studies {.text-sm}

::::: columns
::: {.column width="55%"}
**Aim:** To show that the new treatment is no worse in safety & efficacy than current standard of care.

1.  **Randomized Control Trial**: A 2-arm trial between a new treatment & active control where the new treatment & confidence interval must be within the **non-inferiority margin** of the control's performance.

2.  **Performance Goal Study**: A single-arm trial where results are benchmarked against predefined performance goals. Typical for medical devices as blinding is impractical and/or an active control is financially unjustifiable.
:::
...
## How are Performance Goals Calculated? {.text-sm}
...
Step 2. Conduct an aggregate-level meta-analysis using a 2007 guidance document of FDA-approved stents.
...
A logit-transformation was applied (*extreme proportions*), variance was pooled using a generalised linear mixed model (*robust with small sample sizes*), with study-level variances drawn from each study's observed proportion (*not assuming homogeneity across studies*).
...
# Section 2 {background-color="#5A3A6E"}

-   CI Methods for Binomial Proportions
-   Simulation-Based Performance Evaluation
-   Sample Size Calculation
-   **PG-Power**: An interactive Shiny App
-   Next Steps

## Why Confidence Interval Method Matters

-   The chosen CI method affects the required sample size to achieve adequate power.

-   It must be pre-specified in the Statistical Analysis Plan (SAP)

-   **Companies:** Larger sample sizes increase study costs, resources, and timelines.

-   **Patients:** Larger samples sizes may delay device regulatory approval and patient access to beneficial technology\

![](images/confidenceiskey.jpg)

## The Challenge for Confidence Intervals

-   CI struggle when proportions are extreme.

    **Low Complication:** Observed proportions close to 0

    **High Success:** Observed proportions close to 1

    ![](images/clipboard-4269530673.png){width="450"}
...
-   **Limitations:**
    ...
    -   Assumes $\hat{p}$ follows a normal distriubution

    -   Can produce impossible CI bounds outside the \[0,1\]

    -   Underestimates uncertainty at bounderies

    -   Results in poor coverage, below nominal 95%
:::

::: {.column width="40%"}
![](images/clipboard-1580662855.png){fig-align="center" width="320"}
:::
:::::
...
-   Identifies values of the true proportion $p$ consistent with the observed data

-   Pulls the interval centre away from the boundary ➔ Improves Coverage at boundaries  

## Wilson Score Interval with Continuity Correction

$$\text{Derived by inverting the score test:}
\quad
\frac{(\hat{p} - p)^2}{\hat{p}(1-\hat{p})/n}$$

-   **Prop.test() (R)**

    -   Applys a continuity correction
    -   Adjusts the score test boundary by (0.5)
    -   Produces wider, more conservative intervals

## Agresti-Coull Interval

Modifies the Wald approach by adding pseudo-observations stabilising the estimate.

-   Adding 2 successes and 2 failures (for a 95% CI)

$$\hat{p} = \frac{x}{n}
\qquad \longrightarrow \qquad
\tilde{p} = \frac{x + 2}{n + 4}$$

Then calculates Wald CI:
...
-   The interval is obtained by inverting the exact binomial test:

    $$\left[
    \mathrm{Beta}^{-1}\!\left(\tfrac{\alpha}{2};\; x,\; n - x + 1\right),
    \;\;
    \mathrm{Beta}^{-1}\!\left(1 - \tfrac{\alpha}{2};\; x + 1,\; n - x\right)
    \right]$$ Keep values of $p$ that would **not be rejected** by the observed data

    Guarantees nominal coverage (often slightly conservative)

## Jeffrey Interval

A Bayesian credible interval for a binomial proportion:

$$X \sim \text{Binomial}(n, p), \qquad p \sim \mathrm{Beta}\left(\tfrac{1}{2}, \tfrac{1}{2}\right)$$

Starts with a weak prior and updates with a beta posterior.
...
-   Although Bayesian in construction, the Jeffreys interval often shows strong frequentist performance in terms of coverage, particularly in small samples and boundary cases.
...
*Simulation adapted for performance goal settings: coverage and width evaluated at proportions and sample sizes representative of device non-inferiority trials*

-   Behavior Evaluated for:
...
## Coverage Probability at Low Event Proportions (n = 50)
...
## Expected 95% CI Width: n = 50 vs n = 100
...
**Wald's interval fails** at extreme proportions - undercoverage regardless of $n$
:::
...
**Wilson Score & Jeffreys** better balance coverage accuracy and interval width
:::
...
::: {.column width="50%"}
![](images/clipboard-3302382564.png)
:::
:::::::::

## Sample Size: Safety Endpoint
...
| Power                                      | 90%               |
| Approved Safety Endpoint Proportion (PG)   | 0.11              |
| Assumed Safety Endpoint Proportion ($p_1$) | 0.05              |
...
## Why the saw-tooth shape? {.text-sm}

:::::: columns
::: {.column width="57%"}
It is a discrete fraction: $\frac{\text{critical value}}{n}$, e.g. $\frac{1}{50},\;\frac{1}{51},\;\frac{1}{52},\;\frac{2}{53}$
...
![](images/clipboard-2085374562.png){width="65%"}
:::
...
*https://xj73kq-filip0mikolaj-klosowski.shinyapps.io/shiny_app/*

![](images/clipboard-2974151266.png){width="500"}

## How this relates to Roche?

-   Methodological choices has **consequences**

    -   CI method ➙ Can affect sample size & results

-   Results must be **reproducible and transparent**

    -   Statistical methods ➙ Must be defined, justified & consistent

-   Statistics must be **communicable** for a cross-functional team
...
-   Never rely on a single implementation
    ...
    -   **Cross-validation** is essential
        ...
        ![](images/clipboard-3350543873.png)
...
<h1 class="thank-you-gradient" style="font-size: 90px; background: linear-gradient(90deg, #005EB8, #E41E3F); -webkit-background-clip: text; color: transparent;">
