...
abstract: |
  ...
  randomised active-control trial is impractical or ethically unjustifiable. The conclusion depends on whether sufficient evidence is provided (using a hypothesis test or confidence interval) to satisfy a specific performance goal. Despite 
  ...
  116 patients in total. Confidence interval boundaries rather than point estimates were used
  to account for uncertainty in the historical evidence yielded a maximum acceptable 30-day 
  composite adverse event rate of 12% and a minimum acceptable 12-month Rutherford success 
  ...
  Wald interval was the only method to consistently fall below the nominal 95% coverage level 
  and was judged unsuitable as a decision tool in this setting. Among the remaining methods, 
  Wilson Score and Jeffreys achieved coverage closest to the nominal level with the narrowest 
  intervals, while Clopper-Pearson, Agresti-Coull and the continuity-corrected Wilson Score 
  ...
  size ranged from 198 patients under Jeffreys to 247 under the continuity-corrected Wilson 
  Score.

  An interactive R Shiny application, PG-Power, was developed to implement the sample size 
  framework and make the sensitivity of required sample size to key design assumptions 
  accessible and reproducible. The application supports confidence interval method comparison, 
...
Performance goal studies are essentially another word for a single-arm non-inferiority designs and are particularly common in medical device evaluation [@schumi_through_2011; @wang_leveraging_2021; @wang_single-arm_2024]. Rather than comparing the new intervention against an active control, the observed outcome is compared with a pre-specified benchmark derived from historical evidence and clinical judgement [@wang_leveraging_2021; @rocha-singh_performance_2007]. This benchmark defines the minimum level of performance considered acceptable for the device under evaluation [@wang_leveraging_2021; @rocha-singh_performance_2007
...
Superiority, equivalence and non-inferiority trials address distinct clinical questions and should not be interpreted interchangeably [@lesaffre_superiority_2008; @chow_sample_2017]. Let $p_T$ denote the success probability in the investigational treatment group and $p_C$ denote the success probability in the comparator group. For a binary success endpoint, the treatment effect is defined as [@chow_sample_2017; @schulz_sample_2005]:
...
where $z = z_{1-\alpha/2}$. It generally achieves better coverage behaviour than the Wald interval, especially near the boundaries [@brown_interval_2001].
...
This formulation directly aligns the sample size calculation with the planned primary analysis [@wang_leveraging_2021; @us_food_and_drug_administration_non-inferiority_2016]. If the final decision will be based on a Wilson, Clopper-Pearson or Jeffreys interval, the sample size calculation should use the same method — otherwise the design and analysis are operating under different statistical rules [@wang_leveraging_2021; @brown_interval_2001].
...
```{r sawtooth-power, echo=FALSE, fig.cap="Example saw-tooth power curve illustrating the discrete stepwise behaviour of binomial power calculations for a single-arm binary endpoint. Power is plotted against sample size n. Within each range of n, power decreases gradually before jumping sharply upward when an additional allowable event becomes permissible.", out.width="80%"}
...
The central assumption underlying any borrowing strategy is exchangeability — the requirement that the historical and current populations are sufficiently similar that combining information across them is scientifically justified [@us_food_and_drug_administration_guidance_2010; @chiaruttini_bayesian_2025]. Where full exchangeability cannot be assumed, partial borrowing approaches allow the historical contribution to be downweighted [@ibrahim_power_2000; @chiaruttini_bayesian_2025]. The power prior of Ibrahim and Chen (2000) achieves this by raising the historical likelihood to a discounting factor $a_0 \in [0,1]$ before forming the prior, with $a_0 = 1$ representing full borrowing and $a_0 = 0$ representing no borrowing [@ibrahim_power_2000]. Adaptive borrowing methods take a data-driven approach, adjusting the degree of borrowing based on the observed compatibility between historical and current data — increasing borrowing when the two are consistent and reducing it when they diverge [@us_food_and_drug_administration_guidance_2010; @chiaruttini_bayesian_2025].

Because borrowing affects operating characteristics including Type I error and power, regulatory guidance recommends that its properties be evaluated through simulation across a range of plausible scenarios before the design is finalised [@us_food_and_drug_administration_guidance_2010; @chiaruttini_bayesian_2025; @gamble_guidelines_2017]. The FDA guidance on Bayesian statistics in medical device trials acknowledges the potential for historical borrowing to reduce sample size requirements while emphasising that the assumptions underlying the prior must be clinically and statistically justified [@campbell_bayesian_2011; @us_food_and_drug_administration_guidance_2010]. Section 4.4 applies this framework to the BioMimics 3D safety endpoint using the historical evidence derived in Section 5.1.

## Statistical Analysis Plans in Clinical Trials

A Statistical Analysis Plan (SAP) is a formal document that provides a detailed and technical description of the statistical methods to be used in the analysis of a clinical trial, elaborating on the approaches outlined in the study protocol [@gamble_guidelines_2017; @fradera_statistical_2024]. According to ICH E9, the SAP should be finalised before the unblinded review of data, ensuring that all analytical decisions are made independently of the observed results [@international_council_for_harmonisation_statistical_1998]. This requirement exists because statistical analysis involves choices about endpoints, methods, populations and decision rules that can influence the conclusion of a study, and pre-specifying those choices removes the opportunity for them to be made, consciously or otherwise, in a direction favourable to a particular outcome.

Pre-specification is considered a cornerstone of reproducible clinical research and is expected by regulatory bodies including the FDA and EMA as part of any submission supporting a marketing authorisation [@international_council_for_harmonisation_statistical_1998; @noauthor_medical_2026; @kahan_how_2020]. For randomised controlled trials, the discipline of the randomisation process provides some inherent protection against bias, but the SAP remains essential for ensuring that the primary analysis is defined prospectively and that any deviations from it are transparent and justified [@schulz_sample_2005; @gamble_guidelines_2017].

In single-arm performance goal studies, the SAP carries particular weight. Without a concurrent control group, the trial conclusion depends entirely on how the observed data are compared against a pre-specified benchmark, and the choices embedded in that comparison — namely the performance goal value, the confidence interval method and the decision rule — can each independently alter the conclusion [@wang_leveraging_2021; @cao_estimating_2025; @kahan_how_2020]. A SAP that specifies these choices before data collection provides the primary safeguard against the analytical flexibility that the single-arm design would otherwise permit [@international_council_for_harmonisation_statistical_1998; @wang_single-arm_2024]. The practical implications of these choices for sample size and study feasibility are examined in the context of the BioMimics 3D case study throughout this thesis.

# Case Study and Data Sources

## BioMimics 3D Vascular Stent

The BioMimics 3D vascular stent was selected as the primary case study throughout this analysis. The device is a self-expanding, permanent bare-metal nitinol stent developed for the treatment of femoropopliteal peripheral artery disease, a condition characterised by atherosclerotic narrowing of the arteries supplying the lower limb [@rammos_biomimics_2024; @laird_nitinol_2010; @zeller_helical_2016]. In this setting, successful treatment is judged on achieving initial vessel patency and its ability to maintain clinical benefit over time without restenosis, repeat intervention, amputation or major adverse events [@rocha-singh_performance_2007; @rammos_biomimics_2024; @tepe_local_2008].

Unlike conventional straight stents, the BioMimics 3D stent incorporates a helical geometry intended to mimic the natural curvature and movement of the femoropopliteal artery [@rammos_biomimics_2024; @zeller_helical_2016]. This design is hypothesised to promote blood flow patterns within the treated vessel and to reduce the risk of restenosis after implantation [@rammos_biomimics_2024; @zeller_helical_2016]. Consequently, clinically meaningful outcomes such as target vessel revascularisation, amputation and changes in Rutherford classification are important measures of the device's ability to provide sustained therapeutic benefit in patients with symptomatic peripheral artery disease [@rocha-singh_performance_2007; @rammos_biomimics_2024; @laird_nitinol_2010].

Initial meetings with the lead scientist involved in the BioMimics development helped clarify the selection of appropriate performance goals, the purpose of the device and the statistical issues most relevant to the design of a pivotal study. These discussions informed the choice of safety and efficacy endpoints, the interpretation of clinically meaningful performance, and the practical constraints surrounding sample size. This was important because the statistical design requires clinical knowledge of the device's intended purpose [@schumi_through_2011; @international_council_for_harmonisation_statistical_1998; @wang_leveraging_2021]. Performance goals, endpoints and sample size that are mathematically convenient but clinically weak would not provide a convincing basis for evaluating the device [@international_council_for_harmonisation_statistical_1998; @wang_leveraging_2021; @rocha-singh_performance_2007].
...
For this reason, the proposed performance goals in this study were based on confidence interval boundaries rather than the pooled point estimates. For the safety endpoint, the upper confidence limit was used because it allows for uncertainty around the estimated historical event proportion [@wang_leveraging_2021; @rocha-singh_performance_2007]. For the efficacy endpoint, the lower confidence limit was used because it allows for greater uncertainty around the estimated success proportion [@wang_leveraging_2021; @rocha-singh_performance_2007].
...
To evaluate the confidence interval methods introduced in Section 2.3, a simulation study was designed following the approach of @pires_interval_2008. The aim was to compare coverage probability and expected interval width across a range of true event proportions and sample sizes, before applying these methods within the performance goal framework [@pires_interval_2008].

For each scenario, the number of observed events was assumed to follow a binomial distribution,

$$X \sim \text{Binomial}(n, p)$$

where $X$ is the number of observed events, $n$ is the sample size and $p$ is the true underlying event probability [@brown_interval_2001]. Rather than evaluating performance at a single fixed proportion, a grid of true proportions was used so that interval behaviour could be examined across a range of clinically plausible event rates. Given that the BioMimics 3D endpoints involve either low complication rates or high success rates, the simulation focused on the range [@brown_interval_2001; @pires_interval_2008]:
...
Sample size calculations were carried out for the BioMimics 3D case study using the single-arm performance goal framework described in Section 2.4 [@wang_leveraging_2021; @rocha-singh_performance_2007]. The performance goal values, assumed true event proportion and endpoint were determined from the meta-analysis and clinical discussions described in Sections 3.1 and 3.3, with results reported in Section 5.3.
...
To construct a concrete and interpretable prior, a hypothetical Phase II dataset of 50 participants with 2 observed events was used, producing a $\text{Beta}(2, 48)$ prior with an estimated event proportion of approximately 4% and an effective sample size of 50 [@us_food_and_drug_administration_guidance_2010]. The choice of a hypothetical dataset rather than the historical studies directly was deliberate — the meta-analysis in Section 5.1 identified wide uncertainty around the heterogeneity estimates and only three contributing studies, making a fully justified prior difficult to construct [@ibrahim_power_2000; @chiaruttini_bayesian_2025; @gamble_guidelines_2017]. The hypothetical prior therefore serves as an illustrative example of how borrowing would operate under plausible assumptions, rather than as a formal design recommendation [@campbell_bayesian_2011; @chiaruttini_bayesian_2025].
...
```{r performance-goals-table, echo=FALSE, fig.cap="Proposed performance goals for the BioMimics 3D vascular stent pivotal study derived from meta-analysis of historical femoropopliteal bare nitinol stent studies.", out.width="100%"}
knitr::include_graphics("Figures/performance_goals_table.png")
```

```{r forest-plot, echo=FALSE, fig.cap="Pooled event proportions with 95% confidence intervals for the safety and efficacy endpoints. The diamond denotes the pooled estimate and the dashed red line indicates the derived performance goal.", out.width="90%"}
...
```{r heterogeneity, echo=FALSE, fig.cap="Heterogeneity statistics from the random-effects meta-analysis for the 30-day composite safety endpoint (Table A) and the 12-month Rutherford classification efficacy endpoint (Table B).", out.width="60%"}
knitr::include_graphics("Figures/heterogeneity_statistics.png")
```
...
At $n = 50$, the Wald interval produced coverage below 0.80 for true proportions near zero, only approaching the nominal level near $p = 0.20$. The Wilson Score interval oscillated around 0.95, with dips below 0.90 at very low proportions near $p = 0.001$ to 0.01. The Jeffreys interval followed a similar pattern but with more pronounced dips below nominal at very low proportions. The Wilson Score with continuity correction and Agresti-Coull interval maintained coverage above the nominal level across virtually the entire proportion range. The Clopper-Pearson interval was the most conservative, with coverage remaining above 0.97 across nearly the entire evaluated range.
...
The Wald interval produced the smallest required sample size of 169 patients but achieved power of only 77.3% under the assumed true complication proportion, falling well short of the 90% target. This is consistent with the simulation results in Section 5.2, which showed the Wald interval systematically undercovering across the proportion range relevant to this endpoint [@brown_interval_2001; @agresti_approximate_1998]. A sample size derived from the Wald interval would therefore be unreliable as a basis for study design.

Among the remaining methods, Jeffreys required 198 patients with achieved power of 87.7%, and Wilson Score required 208 patients at 83.9% power, both falling below the 90% target despite requiring more patients than Wald. Clopper-Pearson and Agresti-Coull were the only methods to approach the target, requiring 209 and 210 patients at achieved powers of 89.7% and 89.4% respectively. The Wilson Score with continuity correction required the largest sample size of 247 patients, achieving 92.8% power, consistent with its conservative coverage behaviour observed in Section 5.2.

Notably, Wilson Score required only one fewer patient than Clopper-Pearson but achieved considerably lower power at 83.9% versus 89.7%. This reflects the stepwise nature of the power calculation for binary endpoints discussed in Section 2.4, where a single additional patient can shift the maximum allowable number of events and produce a discrete jump in achieved power [@chow_sample_2017; @brown_interval_2001]. The difference between the smallest reliable sample size and the largest, Clopper-Pearson at 209 and Wilson Score with continuity correction at 247, represents 38 additional evaluable patients or approximately 42 additional enrolments after dropout adjustment, illustrating how method choice directly affects study feasibility [@schulz_sample_2005; @wang_sample_2020].
...
The sensitivity of this result to prior weight is shown in the figure below. All three distributions, $\text{Beta}(0.2, 4.8)$, $\text{Beta}(2, 48)$ and $\text{Beta}(20, 480)$, encode the same underlying event ratio but differ substantially in how tightly they concentrate the posterior.
...
With ESS = 5 the posterior is broad and driven largely by the current data, offering modest sample size reduction relative to the frequentist result. With ESS = 50 the posterior concentrates more clearly around the historical event proportion of 4%, well below the performance goal of 11%, producing the reduction to $n = 79$ reported above. With ESS = 500 the posterior is narrow and the prior dominates, largely overriding the current trial data. While this produces the greatest reduction in required sample size, it also renders the conclusion highly sensitive to the validity of the exchangeability assumption. If the true event proportion in the current study differs from the historical estimate, a strongly informative prior will pull the posterior toward the historical value regardless of what the current data show.
...
This has two practical consequences. First, where within the range the performance goal is set matters: the sensitivity of sample size requirements to the difference between the proposed 12% and approved 11% thresholds is examined in Aim 3. Second, deriving performance goals from conservative confidence interval boundaries rather than point estimates produces a less stringent benchmark, which reduces the risk of an unrealistically strict threshold but introduces the possibility of biocreep [@eversonstewart_biocreep_2010]. If successive device generations are each evaluated against benchmarks derived from increasingly conservative historical estimates, acceptable performance standards may gradually drift from the original level of clinical effectiveness. In a single-arm study without a concurrent control, this risk is amplified because there is no active comparator to anchor the comparison [@wang_leveraging_2021; @cao_estimating_2025]. The validity of the derived benchmarks ultimately depends on clinical and regulatory judgement alongside the statistical evidence, and no amount of methodological care can substitute for that judgement.

### Aim 2: Comparing Confidence Interval Methods for Binary Endpoints

The simulation study confirmed that confidence interval method choice has a meaningful and consistent effect on coverage behaviour at the low event proportions relevant to the BioMimics 3D safety endpoint. The Wald interval was the only method to consistently fall below the nominal 95% level, with average coverage of approximately 0.73 at $n = 20$, 0.84 at $n = 50$ and 0.89 at $n = 100$. Importantly, this undercoverage was not accompanied by any compensating gain in precision: the Wald interval produced average widths comparable to better-performing methods while failing to meet the nominal level throughout. Although its poor behaviour at extreme proportions is well documented in the literature, its inclusion here serves a practical purpose by demonstrating that a method commonly applied by default in standard software would produce unreliable results in exactly this setting [@brown_interval_2001; @agresti_approximate_1998].
...
However, the magnitude of this reduction is sensitive to the weight assigned to the prior. Moving from an effective sample size of 5 to 500 while holding the event ratio constant progressively concentrates the posterior around the historical estimate, reducing the required sample size but increasing dependence on the exchangeability assumption. When the prior is strongly informative and the historical and current populations differ, the posterior can be drawn toward the historical estimate regardless of what the current data show, inflating the Type I error rate in ways that undermine the trial conclusion [@ibrahim_power_2000; @chiaruttini_bayesian_2025; @gamble_guidelines_2017]. The prior weight is therefore a substantive design choice rather than a technical detail.

In the BioMimics 3D context this concern is not hypothetical. The meta-analysis drew on only three studies with wide uncertainty around the heterogeneity estimates, and the exchangeability of that evidence with a future pivotal study cannot be assumed. The reduction from 209 to 79 patients should be understood as an illustration of the method's potential rather than a design recommendation. Realising that potential in practice would require formal exchangeability assessment, sensitivity analyses across a range of prior specifications, and simulation studies evaluating operating characteristics under scenarios where historical and current data diverge [@ibrahim_power_2000; @chiaruttini_bayesian_2025; @gamble_guidelines_2017].
...
The meta-analysis established that the performance goal is a range, not a fixed value. The simulation study showed that the confidence interval method determines both the reliability of the decision rule and the sample size required to achieve a given power. The sample size analysis made this concrete, with an interval-method-driven difference of 38 evaluable patients under identical clinical assumptions. The PG-Power application demonstrated that these calculations can be made transparent and reproducible, and the Bayesian analysis showed that efficiency gains are available from historical borrowing but only when the underlying assumptions are honestly examined.

The thread running through all five aims is the one identified in Section 1.3. These choices are routinely made in performance goal studies, but their rationale is not always made visible. The results of this study suggest that the concern @piaggio_reporting_2012 raised about non-inferiority margin reporting extends equally to the confidence interval method, the sample size procedure and the prior specification. Making these choices explicit, justified and pre-specified is not a procedural formality. It is the mechanism by which a performance goal study earns the credibility needed to support a regulatory decision.
...
The 38-patient difference in required sample size between the most and least demanding reliable confidence interval methods identified in Section 5.3 illustrates concretely what is at stake at the design stage. PG-Power directly supports this process by allowing the sensitivity of sample size requirements to method choice, performance goal value and dropout assumptions to be examined transparently before the SAP is finalised. Documenting these outputs alongside the chosen method and its justification provides a clear and reproducible record of the design rationale, which is precisely what regulators and future reviewers need to evaluate the study's conclusions with confidence [@international_council_for_harmonisation_statistical_1998; @gamble_guidelines_2017].
...
The simulation study showed that confidence interval method choice has a consistent and meaningful effect on coverage behaviour at the low event proportions characteristic of medical device safety endpoints. The Wald interval was the only method to consistently fall below the nominal 95% coverage level and is unsuitable as a decision tool in this setting [@brown_interval_2001]. Wilson Score and Jeffreys achieved coverage closest to the nominal level with the narrowest intervals, while Clopper-Pearson, Agresti-Coull and the continuity-corrected Wilson Score provided conservative coverage at the cost of wider intervals. These differences translated directly into sample size requirements, with the required evaluable sample size ranging from 169 patients under the Wald interval to 247 under the continuity-corrected Wilson Score under identical design assumptions, and Clopper-Pearson requiring 209 patients as the primary result.

The Bayesian historical borrowing analysis demonstrated that incorporating prior evidence through a $\text{Beta}(2, 48)$ prior reduced the required sample size from 209 to 79 patients, a reduction of approximately 62%. This reduction was sensitive to the weight assigned to the prior, and stronger borrowing increased dependence on the exchangeability assumption in ways that risk inflating the Type I error rate. The analysis established the potential of Bayesian borrowing as a complementary approach to the frequentist framework, contingent on careful prior specification and formal exchangeability assessment.
...
Taken together, these findings reinforce a conclusion that extends beyond the BioMimics 3D case study. The concern raised in Section 1.3, that performance goals and statistical methods are often presented without explanation, applies not only to the benchmark itself but to the confidence interval method, the sample size procedure and the prior specification [@piaggio_reporting_2012]. Each of these choices shapes the trial conclusion before a patient is enrolled, and making them explicit, justified and pre-specified in the SAP is not a procedural formality. It is the foundation on which a credible and reproducible performance goal study rests.
